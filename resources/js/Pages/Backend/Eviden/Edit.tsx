import React, { useState, useEffect } from "react";
import MainLayout from "@/Layouts/MainLayout";
import Card from "@/Components/UI/Card";
import Button from "@/Components/UI/Button";
import TextInput from "@/Components/Form/TextInput";
import SelectInput from "@/Components/Form/SelectInput";
import { Head, useForm, Link, router, usePage } from "@inertiajs/react";
import {
    ArrowLeft,
    Save,
    Upload,
    Info,
    Trash2,
    Eye,
    Download,
    CheckCircle,
    XCircle,
    Clock,
} from "lucide-react";
import axios from "axios";
import ConfirmationModal from "@/Components/UI/ConfirmationModal";

interface FileEviden {
    id: number;
    file_path: string;
    original_name: string;
    file_type: string;
}

interface EditProps {
    eviden: {
        id: number;
        uuid: string;
        indikator_id: number;
        skpd_id: number;
        tahun: string;
        status: string;
        files: FileEviden[];
        indikator: {
            nama_indikator: string;
        };
    };
    indikators: { id: number; nama_indikator: string }[];
    skpds: { id: number; nama_skpd: string }[];
}

interface IndikatorDetail {
    indikator_id: number;
    nama_indikator: string;
    deskripsi: {
        tingkat: string;
        kriteria_kenaikan_tingkat: string;
    }[];
}

export default function Edit({ eviden, indikators, skpds }: EditProps) {
    const { data, setData, post, processing, errors, reset } = useForm({
        _method: "PUT",
        indikator_id: eviden.indikator_id,
        skpd_id: eviden.skpd_id,
        tahun: eviden.tahun,
        status: eviden.status,
        files: [] as File[],
    });

    const [indikatorDetail, setIndikatorDetail] =
        useState<IndikatorDetail | null>(null);
    const [isLoadingDetail, setIsLoadingDetail] = useState(false);
    const [fileToDelete, setFileToDelete] = useState<FileEviden | null>(null);
    const [showDeleteModal, setShowDeleteModal] = useState(false);

    const { auth } = usePage().props as any;
    const isAdmin = auth.roles && auth.roles.includes("admin");

    useEffect(() => {
        if (data.indikator_id) {
            setIsLoadingDetail(true);
            axios
                .get(route("be.api.indikator.detail", data.indikator_id))
                .then((res) => {
                    setIndikatorDetail(res.data.data);
                })
                .catch((err) => {
                    console.error("Failed to fetch indikator detail", err);
                    setIndikatorDetail(null);
                })
                .finally(() => {
                    setIsLoadingDetail(false);
                });
        }
    }, [data.indikator_id]);

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route("be.eviden.update", eviden.uuid), {
            forceFormData: true,
        });
    };

    const confirmDeleteFile = (file: FileEviden) => {
        setFileToDelete(file);
        setShowDeleteModal(true);
    };

    const handleDeleteFile = () => {
        if (fileToDelete) {
            router.delete(route("be.eviden.file.delete", fileToDelete.id), {
                onSuccess: () => {
                    setShowDeleteModal(false);
                    setFileToDelete(null);
                    // Page will automatically reload props, updating the file list
                },
            });
        }
    };

    return (
        <>
            <Head title="Edit Data Eviden" />

            {/* Header */}
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-200">
                        Edit Data Eviden
                    </h1>
                    <p className="text-slate-500 text-sm dark:text-slate-400">
                        Perbarui bukti dukung SPBE
                    </p>
                </div>
                <Link href={route("be.eviden.index")}>
                    <Button variant="secondary">Kembali</Button>
                </Link>
            </div>

            <div className="flex flex-col lg:flex-row gap-6 items-start">
                <div className="w-full lg:w-2/3">
                    <Card className="w-full p-6 dark:bg-slate-800 dark:border-slate-700">
                        <form onSubmit={handleSubmit} className="space-y-3">
                            {/* SKPD */}
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2 dark:text-slate-300">
                                    Pilih SKPD{" "}
                                    <span className="text-rose-500">*</span>
                                </label>
                                <SelectInput
                                    value={data.skpd_id}
                                    onChange={(e) =>
                                        setData(
                                            "skpd_id",
                                            Number(e.target.value),
                                        )
                                    }
                                    error={errors.skpd_id}
                                    disabled={!isAdmin}
                                >
                                    <option value="">-- Pilih SKPD --</option>
                                    {skpds.map((s) => (
                                        <option key={s.id} value={s.id}>
                                            {s.nama_skpd}
                                        </option>
                                    ))}
                                </SelectInput>
                            </div>

                            {/* Indikator */}
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2 dark:text-slate-300">
                                    Pilih Indikator{" "}
                                    <span className="text-rose-500">*</span>
                                </label>
                                <SelectInput
                                    value={data.indikator_id}
                                    onChange={(e) =>
                                        setData(
                                            "indikator_id",
                                            Number(e.target.value),
                                        )
                                    }
                                    error={errors.indikator_id}
                                >
                                    <option value="">
                                        -- Pilih Indikator --
                                    </option>
                                    {indikators.map((ind) => (
                                        <option key={ind.id} value={ind.id}>
                                            {ind.nama_indikator}
                                        </option>
                                    ))}
                                </SelectInput>
                            </div>

                            {/* Tahun Dropdown */}
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2 dark:text-slate-300">
                                    Tahun{" "}
                                    <span className="text-rose-500">*</span>
                                </label>
                                <SelectInput
                                    value={data.tahun}
                                    onChange={(e) =>
                                        setData("tahun", e.target.value)
                                    }
                                    error={errors.tahun}
                                >
                                    <option value="2024">2024</option>
                                    <option value="2025">2025</option>
                                    <option value="2026">2026</option>
                                </SelectInput>
                            </div>

                            {/* Status (Admin Only) */}
                            {isAdmin && (
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-3 dark:text-slate-300">
                                        Status Verifikasi
                                    </label>
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                        {/* Pending */}
                                        <div
                                            onClick={() =>
                                                setData("status", "pending")
                                            }
                                            className={`cursor-pointer rounded-xl border p-3 flex items-center gap-3 transition-all duration-200 ${
                                                data.status === "pending"
                                                    ? "bg-amber-50 border-amber-500 ring-1 ring-amber-500 text-amber-900 dark:bg-amber-900/20 dark:text-amber-100"
                                                    : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400"
                                            }`}
                                        >
                                            <div
                                                className={`p-1.5 rounded-lg flex-shrink-0 ${
                                                    data.status === "pending"
                                                        ? "bg-amber-100 text-amber-600 dark:bg-amber-900/50 dark:text-amber-400"
                                                        : "bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-500"
                                                }`}
                                            >
                                                <Clock size={18} />
                                            </div>
                                            <div className="font-semibold text-sm">
                                                Pending
                                            </div>
                                        </div>

                                        {/* Terverifikasi */}
                                        <div
                                            onClick={() =>
                                                setData(
                                                    "status",
                                                    "terverifikasi",
                                                )
                                            }
                                            className={`cursor-pointer rounded-xl border p-3 flex items-center gap-3 transition-all duration-200 ${
                                                data.status === "terverifikasi"
                                                    ? "bg-emerald-50 border-emerald-500 ring-1 ring-emerald-500 text-emerald-900 dark:bg-emerald-900/20 dark:text-emerald-100"
                                                    : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400"
                                            }`}
                                        >
                                            <div
                                                className={`p-1.5 rounded-lg flex-shrink-0 ${
                                                    data.status ===
                                                    "terverifikasi"
                                                        ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-900/50 dark:text-emerald-400"
                                                        : "bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-500"
                                                }`}
                                            >
                                                <CheckCircle size={18} />
                                            </div>
                                            <div className="font-semibold text-sm">
                                                Terverifikasi
                                            </div>
                                        </div>

                                        {/* Ditolak */}
                                        <div
                                            onClick={() =>
                                                setData("status", "ditolak")
                                            }
                                            className={`cursor-pointer rounded-xl border p-3 flex items-center gap-3 transition-all duration-200 ${
                                                data.status === "ditolak"
                                                    ? "bg-rose-50 border-rose-500 ring-1 ring-rose-500 text-rose-900 dark:bg-rose-900/20 dark:text-rose-100"
                                                    : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400"
                                            }`}
                                        >
                                            <div
                                                className={`p-1.5 rounded-lg flex-shrink-0 ${
                                                    data.status === "ditolak"
                                                        ? "bg-rose-100 text-rose-600 dark:bg-rose-900/50 dark:text-rose-400"
                                                        : "bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-500"
                                                }`}
                                            >
                                                <XCircle size={18} />
                                            </div>
                                            <div className="font-semibold text-sm">
                                                Ditolak
                                            </div>
                                        </div>
                                    </div>
                                    {errors.status && (
                                        <div className="text-rose-500 text-sm mt-1">
                                            {errors.status}
                                        </div>
                                    )}
                                </div>
                            )}

                            {/* Existing Files */}
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2 dark:text-slate-300">
                                    File Tersimpan
                                </label>
                                {eviden.files && eviden.files.length > 0 ? (
                                    <div className="space-y-2">
                                        {eviden.files.map((file) => (
                                            <div
                                                key={file.id}
                                                className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-200 dark:border-slate-700"
                                            >
                                                <div className="flex items-center gap-3 overflow-hidden">
                                                    <div className="p-2 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg">
                                                        <Info size={20} />
                                                    </div>
                                                    <a
                                                        href={`/storage/${file.file_path}`}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        className="text-sm font-medium text-slate-700 dark:text-slate-300 truncate hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
                                                    >
                                                        {file.original_name}
                                                    </a>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    {/* View Button */}
                                                    <a
                                                        href={`/storage/${file.file_path}`}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        className="p-2 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors"
                                                        title="Lihat file"
                                                    >
                                                        <Eye size={18} />
                                                    </a>

                                                    {/* Download Button */}
                                                    <a
                                                        href={route(
                                                            "be.eviden.file.download",
                                                            file.id,
                                                        )}
                                                        className="p-2 text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg transition-colors"
                                                        title="Download file"
                                                    >
                                                        <Download size={18} />
                                                    </a>

                                                    {/* Delete Button */}
                                                    <button
                                                        type="button"
                                                        onClick={() =>
                                                            confirmDeleteFile(
                                                                file,
                                                            )
                                                        }
                                                        className="p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg transition-colors"
                                                        title="Hapus file"
                                                    >
                                                        <Trash2 size={18} />
                                                    </button>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <p className="text-sm text-slate-500 italic">
                                        Belum ada file yang diupload.
                                    </p>
                                )}
                            </div>

                            {/* New File Upload */}
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2 dark:text-slate-300">
                                    Upload File Baru (Maksimal 10 file
                                    sekaligus)
                                </label>
                                <div className="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl p-8 text-center hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer relative">
                                    <input
                                        type="file"
                                        multiple
                                        onChange={(e) => {
                                            const files = Array.from(
                                                e.target.files || [],
                                            );
                                            if (files.length > 10) {
                                                alert(
                                                    "Maksimal 10 file per upload!",
                                                );
                                                e.target.value = ""; // Reset input
                                            } else {
                                                setData("files", files);
                                            }
                                        }}
                                        className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                                    />
                                    <div className="flex flex-col items-center justify-center text-slate-500 dark:text-slate-400 pointer-events-none">
                                        <div className="p-3 bg-indigo-50 dark:bg-slate-700 rounded-full mb-3 text-indigo-600 dark:text-indigo-400">
                                            <Upload size={24} />
                                        </div>
                                        <span className="font-medium text-slate-700 dark:text-slate-300">
                                            Klik untuk tambah file
                                        </span>
                                        <span className="text-sm mt-1">
                                            atau drag and drop file di sini
                                        </span>
                                        <span className="text-xs text-slate-400 mt-2">
                                            Format: PDF, JPG, PNG, DOC (Max
                                            10MB/file)
                                        </span>
                                    </div>
                                </div>

                                {/* New File Preview List */}
                                {data.files.length > 0 && (
                                    <div className="mt-4 space-y-2">
                                        <p className="text-sm font-medium text-slate-700 dark:text-slate-300">
                                            File Baru yang dipilih:
                                        </p>
                                        <ul className="text-sm text-slate-500 space-y-1 bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                                            {data.files.map((file, index) => (
                                                <li
                                                    key={index}
                                                    className="flex items-center gap-2"
                                                >
                                                    <span className="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                                                    {file.name} (
                                                    {(file.size / 1024).toFixed(
                                                        0,
                                                    )}{" "}
                                                    KB)
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                                )}

                                {errors.files && (
                                    <div className="text-rose-500 text-sm mt-1">
                                        {errors.files}
                                    </div>
                                )}
                            </div>

                            {/* Submit Button */}
                            <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-700">
                                <Button
                                    variant="primary"
                                    type="submit"
                                    disabled={processing}
                                    className="w-full md:w-auto"
                                >
                                    {processing
                                        ? "Menyimpan Perubahan"
                                        : "Simpan Perubahan"}
                                </Button>
                            </div>
                        </form>
                    </Card>
                </div>

                {/* Info Detail Indikator - Right Sidebar */}
                <div className="w-full lg:w-1/3">
                    <Card className="w-full h-full p-6 dark:bg-slate-800 dark:border-slate-700 flex flex-col">
                        <div className="flex items-center gap-2 mb-4 text-indigo-600 dark:text-indigo-400">
                            <Info size={20} />
                            <h3 className="font-semibold text-lg">
                                Informasi Indikator
                            </h3>
                        </div>

                        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                            {isLoadingDetail ? (
                                <div className="flex flex-col items-center justify-center h-40 text-slate-400 animate-pulse">
                                    <div className="w-8 h-8 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin mb-2"></div>
                                    <p>Memuat informasi...</p>
                                </div>
                            ) : indikatorDetail ? (
                                <div className="space-y-4">
                                    <div>
                                        <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">
                                            Nama Indikator
                                        </span>
                                        <p className="text-slate-700 dark:text-slate-300 font-medium text-sm mt-1">
                                            {indikatorDetail.nama_indikator}
                                        </p>
                                    </div>

                                    <div>
                                        <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-2">
                                            Penjelasan Level
                                        </span>
                                        <div className="space-y-3">
                                            {indikatorDetail.deskripsi.map(
                                                (desc, idx) => (
                                                    <div
                                                        key={idx}
                                                        className="p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-100 dark:border-slate-700/50"
                                                    >
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 rounded text-xs font-bold text-sm">
                                                                Level{" "}
                                                                {desc.tingkat}
                                                            </span>
                                                        </div>
                                                        <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                                                            {
                                                                desc.kriteria_kenaikan_tingkat
                                                            }
                                                        </p>
                                                    </div>
                                                ),
                                            )}
                                        </div>
                                    </div>
                                </div>
                            ) : (
                                <div className="text-center text-slate-400 py-10">
                                    <p>
                                        Pilih indikator terlebih dahulu untuk
                                        melihat informasi detail.
                                    </p>
                                </div>
                            )}
                        </div>
                    </Card>
                </div>
            </div>

            <ConfirmationModal
                show={showDeleteModal}
                title="Hapus File"
                message={`Apakah Anda yakin ingin menghapus file "${fileToDelete?.original_name}"?`}
                onConfirm={handleDeleteFile}
                onCancel={() => {
                    setShowDeleteModal(false);
                    setFileToDelete(null);
                }}
            />
        </>
    );
}

Edit.layout = (page: React.ReactNode) => <MainLayout>{page}</MainLayout>;
